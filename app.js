  ...
  if (loggedIn == false) {
    ...
    if (current_user.withdrawals <= 2 && current_user.fines < 1000) {
      ...
      database.query(sql, function(err, result) {
        ...
        if (found == false) {
          canIssue == 2;
          var sql4 = `insert into on_holds(ISBN, user_id) values("${isbnis}", "${current_user.id}");`;
          ...
          if (current_user.type == 0) {
            ...
            database.query(sql3, function(err3, result3) {
              if (err3) throw err3;
            });
          }
          res.render("issue", {
...
app.post("/adminlogin", function(req, res) {
  ...
  var query1 = `SELECT * FROM librarians WHERE lib_email="${adminUser}" AND lib_pwd="${adminPass}"`;
  database.query(query1, function(err, result) {
    if (err) throw err;
    if (result.length == 0) {
      loggedIn = false;
...
app.post("/returnBook", function(req, res) {
  var ISBN = req.body.ISBN;
  var copy_number = req.body.copy_number;
  var current_date = new Date();
  console.log(ISBN, copy_number);
  var sql = `update books set current_status = 2, return_date = now() where ISBN = "${ISBN}" and book_copy = "${copy_number}";`;
  database.query(sql, function(err, result) {
    if (err) throw err;
    res.redirect("/admindashboard");
  })
...
var onholdsarr = [];
app.get("/onholds", function(req, res) {
  res.render("onholds", {onholdsarr: onholdsarr});
})
app.post("/onholds", function(req, res) {
  var ISBN = req.body.ISBN;
  onholdsarr = [];
  var sql = `select user_name from users where user_id in(select user_id from on_holds where ISBN = "${ISBN}");`;
  database.query(sql, function(err, result) {
    if(err) throw err;
    for(var i = 0; i < result.length; i++) {
      onholdsarr.push(result[i].user_name);
    }
    res.redirect("/onholds");
  })
})
